...
class CorrespondenceBasedPipeline(object):
    ...
    def set_target(self, image: np.ndarray, depth: np.ndarray, dist_scale: float, **kwargs):
        """
        Arguments
        ...
        """
        success = self.frontend.update_target_frame(image, depth)
        self.new_scene = True
        self.dist_scale = dist_scale
        self.target_kwargs = kwargs
        return success
    
    def get_control_rate(self, image: np.ndarray, depth: np.ndarray):
        """
        Arguments:
        - image: np.ndarray, bgr image (Note: here is bgr not rgb)
        - depth: np.ndarray, depth image [intrinsic.width, intrinsic.height]
        ...
        t0 = time.time()
        corr = self.frontend.process_current_frame(image, depth)
        ...
        if corr is None:
            ...
            if self.vis & VisOpt.GRAPH:
                # graph = show_graph(data, cv2.applyColorMap(depth, cv2.COLORMAP_JET))
                graph = show_graph(data, depth)
            if self.vis & VisOpt.MATCH:
                match = show_corr(corr, show_keypoints=True)
        ...
        if not isinstance(graph, np.ndarray):
            graph = image = np.ones((480, 640, 3), dtype=np.uint8) * 64
        if not isinstance(match, np.ndarray):
            match = np.ones((480, 640 *2 +10, 3), dtype=np.uint8) * 64

        timing["total_time"] = timing["frontend_time"] + timing["backend_time"]
        return vel, data, timing, match, graph
