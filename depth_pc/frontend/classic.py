...
class Classic(FrontendBase):
    ...
    def bilinear_interpolation(self, depth, pos):
        x, y = pos
        x1, y1= int(x), int(y)
        x2, y2 = min(x1 + 1, depth.shape[1] - 1), min(y1 + 1, depth.shape[0] - 1)

        d1, d2, d3,d4 = depth[y1, x1], depth[y1, x2], depth[y2, x1], depth[y2, x2]
        interpolated_depth = (d1 * (x2 - x) * (y2 - y) +
                              d2 * (x - x1) * (y2 - y) +
                              d3 * (x2 - x) * (y - y1) +
                              d4 * (x - x1) * (y - y1)) 
        return interpolated_depth

    def get_depth(self, depth: np.ndarray, pos):
        depth = np.array([self.bilinear_interpolation(depth, pos[i]) 
                          for i in range(pos.shape[0])])
        return depth

    def update_target_frame(self, image, depth, mask=None):
        tar_pos = []
        tar_des_rot = []
        ...
        if success_any:
            self.tar_img = image
            self.tar_img_changed = True
            self.tar_des_rot = np.concatenate(tar_des_rot) # [n_tar_feats, 61]
            self.tar_pos = np.concatenate(tar_pos) # [n_tar_kp, 2]
            tar_depth = self.get_depth(depth, self.tar_pos)
            self.tar_depth= (tar_depth - tar_depth.min()) / (tar_depth.max() - tar_depth.min())
            self._build_tree()
    ...
    def _align_cur_to_tar(self, cur_pos, matches):
        ...
        cur_pos_aligned[tar_indices] = cur_pos[cur_indices] # find current aligned points in the target
        valid_mask = np.zeros(self.tar_pos.shape[0], dtype=bool)
        valid_mask[tar_indices] = True
        return cur_pos_aligned, valid_mask

    def process_current_frame(self, image, depth):
        ...
        cur_depth = self.get_depth(depth, cur_pos_aligned)
        cur_depth = (cur_depth-cur_depth.min()) / (cur_depth.max()-cur_depth.min())
        corr = Correspondence(
