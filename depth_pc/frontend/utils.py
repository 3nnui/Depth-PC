...
class Correspondence(object):
    ...
    cur_depth: np.ndarray # (H, W) range 0 ~ 1
    ...
    def concat(cls, corr: List["Correspondence"]):
        ...
        if len(valid_corr) == 0:
            ...
            return Correspondence(
                ...
                cur_depth=np.concatenate([c.cur_depth for c in valid_corr], axis=0),
...
class FrontendConcatWrapper(object):
    ...
    def process_current_frame(self, image, depth):
        corr = [f.process_current_frame(image, depth) for f in self.frontends]
...
def show_corr(corr: Correspondence, color=None, show_keypoints=False, margin=10):
    ...
    cv2.imshow("Target | Current", out[:, :, [2, 1, 0]])
    cv2.waitKey(1)
    return out[:, :, [2, 1, 0]]
