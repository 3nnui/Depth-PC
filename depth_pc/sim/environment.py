...
from depth_pc.utils.perception import Camera, CameraIntrinsic

from .sampling import (gen_virtual_points, gen_virtual_points_uniformed,
                       sample_camera_pose, uniform_ball_sample)
        ...
        if self.resample or (self.points is None):
            ...
            while True:
                ...
                if not self.close_enough(self.target_wcT, self.initial_wcT):
                    break
        target_points, target_indices = self.camera.filter_occlu(self.target_wcT, self.points)
        current_points, current_indices = self.camera.filter_occlu(self.initial_wcT, self.points)
        aligned_indices = np.intersect1d(current_indices, target_indices)
        self.points = self.points[aligned_indices]
