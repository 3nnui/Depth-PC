...
def load_points(models_folder):
    object_dirs = [os.path.join(models_folder, d) for d in os.listdir(models_folder)
    ...
    for obj_dir in object_dirs:
        for filename in os.listdir(obj_dir):
            if filename.endswith('.xyz'):
                ...
                downpcd.scale(0.5, (0,0,0))
                points = np.asarray(downpcd.points)
                points_list.append(points)
    return points_list

def generate_cluster(num_clustered_pts, num_clusters, obs_r, obs_h):
    points_list = load_points("ycbv_models")
    ...
    add_indices = np.random.choice(num_clusters, size=num_clustered_pts, replace=True)
    np.add.at(num_points_per_cluster, add_indices, 1)
    selected_points =[]
    for i in range(num_clusters):
        object_index = random.randint(0, len(points_list) - 1)
        points = points_list[object_index]  
        selected_indices = random.sample(range(len(points)), 21)
        per_clu_points = points[selected_indices]
        drz_max = 180  # degree
        ...
        selected_points.append(per_clu_points @ dpose.T + cluster_centers[i])
    return np.asarray(selected_points).reshape(-1, 3)
...
def gen_virtual_points(num_pts, obs_r, obs_h) -> np.ndarray:
    ...
    if num_pts < 20:
        ...
        clustered_pts = generate_cluster(num_clustered_pts, num_clusters, obs_r, obs_h)
